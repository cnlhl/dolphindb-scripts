...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--savepath", default="/mnt/SandboxData/Level2CSV/order")
    args = parser.parse_args()

    start_day = 20211105
    end_day = 20211231
    target_days = [20220114, 20220207, 20220425, 20220628, 20220629, 20221116, 20221117]
    ...
    for today in datedir:
         if int(today) in target_days:
            ...
            for main_stock in order_reader.get_stock_pool():
                print("Processing order:",main_stock,' ',today)
                main_order = order_reader.get_stock_dataframe(int(main_stock))
                main_order.to_csv(savepath+'/'+str(today)+'/{:06d}.csv'.format(int(main_stock)),index = False)
