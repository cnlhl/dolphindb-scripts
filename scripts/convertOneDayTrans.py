...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--savepath", default="/mnt/SandboxData/Level2CSV/trans")
    ...
    for today in datedir:
        if int(today) >=start_day and int(today) <=end_day:
            ...
            for main_stock in trans_reader.get_stock_pool():
                print("Processing transMarket:",main_stock,' ',today)
                main_trans = trans_reader.get_stock_dataframe(int(main_stock))
                main_trans.to_csv(savepath+'/'+str(today)+'/{:06d}.csv'.format(int(main_stock)),index = False)
